...
function [x1, iterations, all_roots, all_functions] = newton_raphson (x0, tolerance, max_iterations)
  ...
  all_roots = zeros (max_iterations+1, length(x0));
  all_functions = zeros (max_iterations+1, lengh(x0));

  all_roots(1, :) = x0;
  all_functions(1, :) = myfunctions(x0);
  ...
  for iterations = 1 : max_iterations
    ...
    all_functions(iterations+1, :) = myfunctions(x1);
  ...
  all_roots = all_roots (1 : iterations+1, :);
  all_functions = all_functions(1 : iterations+1, :);
end
